...
FROM Students
         LEFT JOIN StudentPrograms ON (idnr = StudentPrograms.student)
         LEFT JOIN StudentBranches ON (idnr = StudentBranches.student);

CREATE VIEW FinishedCourses AS
SELECT student, course, Courses.name AS courseName, grade, credits
FROM Students
         INNER JOIN Taken ON (Students.idnr = Taken.student)
         JOIN Courses ON (Courses.code = Taken.course);
...
WHERE StudentBranches.branch = Recommended.branch
  AND StudentBranches.program = Recommended.program
...
FROM StudentBranches
         LEFT JOIN BranchMandatory ON (StudentBranches.program = BranchMandatory.program AND
                                       StudentBranches.branch = BranchMandatory.branch)
         LEFT JOIN PassedCourses ON (StudentBranches.student = PassedCourses.student AND
                                     PassedCourses.course = BranchMandatory.course)
WHERE (PassedCourses.grade IS NULL)
ORDER BY student;
...
WITH TotalCredits AS
     ...
     MathCredits AS
         (SELECT student,
          ...
          FROM PassedCourses
                   JOIN ClassifiedCourses USING (course)
